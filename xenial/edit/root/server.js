...
const dev = !/(up|wmes|dell|hp)/i.test(os.hostname());
const vm = /^VM/i.test(os.hostname());

const PORT = dev ? 1336 : 80;
...
server.on('request', function(req, res)
{
  ...
  res.setHeader('Access-Control-Allow-Origin', origin || 'https://ket.wmes.pl');
  ...
  {
    return require('./server/trw').handleRequest(req, res);
  }
